    ...
    def __init__(self,block,layers,num_classes=1000):
        ...
        for n,m in self.cells_and_names():
            ...
            if isinstance(m,nn.Conv2d):
                ...
                heNormal=HeNormal(mode="fan_in", nonlinearity="relu")
                new_weight=heNormal(c_weight)
                m.weight.set_data(Tensor(new_weight))
                
            elif isinstance(m,nn.BatchNorm2d):
                
                b_gamma=m.gamma.asnumpy()
                b_constant=Constant(1)
                new_gamma=b_constant(b_gamma)
                m.gamma.set_data(Tensor(new_gamma))

                b_beta=m.beta.asnumpy()
                b_constant=Constant(0)
                new_beta=b_constant(b_beta)
                m.beta.set_data(Tensor(new_beta))
